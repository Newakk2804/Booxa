document.querySelectorAll('input[name="genre"]').forEach(input => {
  input.addEventListener('change', async (e) => {
    ...
    try {
      const response = await fetch(`/search-by-genre?genre=${encodeURIComponent(genre)}`);
      const books = await response.json();

      const container = document.getElementById('list-card');
      container.innerHTML = ''; // Очистить старые результаты

      if (books.length === 0) {
        container.innerHTML = '<p>Нет книг по выбранному жанру.</p>';
        return;
      }
